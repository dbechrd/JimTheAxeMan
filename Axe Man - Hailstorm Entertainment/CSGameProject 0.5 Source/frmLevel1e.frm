...
Begin VB.Form frmLevel1e 
   ...
   Begin VB.Image Image1 
      Height          =   9000
      Left            =   0
      Picture         =   "frmLevel1e.frx":1B31
      Top             =   0
      Visible         =   0   'False
      Width           =   12000
   End
End
Attribute VB_Name = "frmLevel1e"
...
Public Sub Form_Load()
    ...
    'NEWFORM
    If (previousLevel = 1.4) Then
        Unload frmLevel1d
    ElseIf (previousLevel = 1.6) Then
        Unload frmLevel1f
    End If
    ...
    CreateSpriteResource cResEnemy1, App.Path & "\Tree1.bmp", 2, 1, &HFF00FF
    ...
    If (ItemObj.UBound > 0) Then
        ...
        Do While i <= ItemObj.UBound
            ...
            End If
            'NEWFORM
            If Level1x5_IntroText = False Then
                Dim DeclareItemArray As Integer
                'NEWFORM
                ReDim Level1x5_Items(0 To i)
                'NEWFORM
                For DeclareItemArray = 0 To UBound(Level1x5_Items)
                    'NEWFORM
                    Level1x5_Items(DeclareItemArray).FileName = ItemObj(DeclareItemArray).Caption
                    Level1x5_Items(DeclareItemArray).ItemType = Mid(ItemObj(DeclareItemArray).Caption, 1, Len(ItemObj(DeclareItemArray).Caption) - 4)
                    'Level1x1_Items(DeclareItemArray).ItemType = ItemObj(DeclareItemArray).Caption
                    Level1x5_Items(DeclareItemArray).Used = False
            ...
            'End If
            'NEWFORM
            If (Level1x5_Items(i).Used = False) Then
                ...
                cItemObj(i).Cell = 1
            'NEWFORM
            ElseIf (Level1x5_Items(i).Used = True) Then
                CreateSpriteResource cResItemObj(i), App.Path & "\" & ItemObj(i).Caption, 2, 1, &HFF00FF
                CreateSprite cResItemObj(i), cItemObj(i)
                cItemObj(i).X = ItemObj(i).Left \ Screen.TwipsPerPixelX
                cItemObj(i).Y = ItemObj(i).Top \ Screen.TwipsPerPixelY
...
Public Sub GameLoop()
    ...
    If (previousLevel = 1.4) Then
        ...
        'MsgBox ("prlv 14")
    ElseIf (previousLevel = 1.6) Then
        'MsgBox ("lw: " & lW & " - " & cChar.Width & " - 30")
        cChar.X = (lW - cChar.Width - 200)
        cChar.Y = (groundtop - cChar.Height)
        cChar.Cell = 8
        'MsgBox ("prlv 16" & cChar.X & " " & cChar.Y)
    End If
...
Public Function charCollision() As Boolean
    ...
    charOnPlatform = False
    Dim i As Integer
    If cChar.X + cChar.Width > 600 Or cChar.X < 0 Then
        If cChar.X + cChar.Width > 600 Then
...
Public Function UseItem(ObjCollidedWith As Integer)
    ...
    'NEWFORM
    itemname = Level1x5_Items(ObjCollidedWith).ItemType
    ...
    Select Case itemname
        Case "chest"
            ...
            'NEWFORM
            If (Level1x5_Items(ObjCollidedWith).Used = True) Then
            ...
            'NEWFORM
            ElseIf (Level1x5_Items(ObjCollidedWith).Used = False) Then
                ...
                'Call showUserText("The chest was empty.", 2000)
                'NEWFORM
                Level1x5_Items(ObjCollidedWith).Used = True
            End If
        Case "healthbox"
            'healthbox used
...
Private Sub tmrEnemyAttack_Timer()
    ...
    If (cEnemy1.X < cChar.X + cChar.Width And cEnemy1.X + cEnemy1.Width > cChar.X And cEnemy1.Y < cChar.Y + cChar.Height And cEnemy1.Y + cEnemy1.Height > cChar.Y) Then
        charHealth = charHealth - 1
        Call showUserText("You took 1 damage from Tree.", 2000)
    End If
