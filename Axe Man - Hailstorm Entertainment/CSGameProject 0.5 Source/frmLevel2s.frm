...
Begin VB.Form frmLevel2s 
   ...
   Begin VB.Image Image1 
      Height          =   9000
      Left            =   0
      Picture         =   "frmLevel2s.frx":1B31
      Top             =   0
      Visible         =   0   'False
      Width           =   13440
   End
End
Attribute VB_Name = "frmLevel2s"
...
Dim Cheat_superjump As Boolean, Cheat_teabag As Boolean, Cheat_speed As Boolean, Cheat_swirl As Boolean, Cheat_nme As Boolean
...
Public Sub Form_Load()
    ...
    'CreateSpriteResource cResChar, App.Path & "\boss_final.bmp", 14, 1, &HFF00FF
    CreateSprite cResChar, cChar
    
    CreateSpriteResource cResEnemy1, App.Path & "\boss_final.bmp", 14, 1, &HFF00FF
    ...
    If (ItemObj.UBound > 0) Then
        ...
        Do While i <= ItemObj.UBound
            'NEWFORM
            If Level2xS_IntroText = False Then
                Call showUserText("May the force be with you...", 3000)
                charHealth = 100
                enemy1Health = 100
                '===============================
                Dim DeclareItemArray As Integer
                'NEWFORM
                ReDim Level2xS_Items(0 To i)
                'NEWFORM
                For DeclareItemArray = 0 To UBound(Level2xS_Items)
                    'NEWFORM
                    Level2xS_Items(DeclareItemArray).FileName = ItemObj(DeclareItemArray).Caption
                    Level2xS_Items(DeclareItemArray).ItemType = Mid(ItemObj(DeclareItemArray).Caption, 1, Len(ItemObj(DeclareItemArray).Caption) - 4)
                    'Level1x1_Items(DeclareItemArray).ItemType = ItemObj(DeclareItemArray).Caption
                    Level2xS_Items(DeclareItemArray).Used = False
            ...
            'End If
            'NEWFORM
            If (Level2xS_Items(i).Used = False) Then
                ...
                cItemObj(i).Cell = 1
            'NEWFORM
            ElseIf (Level2xS_Items(i).Used = True) Then
                CreateSpriteResource cResItemObj(i), App.Path & "\" & ItemObj(i).Caption, 2, 1, &HFF00FF
                CreateSprite cResItemObj(i), cItemObj(i)
                cItemObj(i).X = ItemObj(i).Left \ Screen.TwipsPerPixelX
                cItemObj(i).Y = ItemObj(i).Top \ Screen.TwipsPerPixelY
...
Public Sub GameLoop()
    ...
    Do While bGameLoop
        ...
        If (enemyExists = True And (enemy1Health <= 0 Or Cheat_nme)) Then
            ScreenNeedsUpdate = True
            enemyExists = False
            tmrEnemyMove.Enabled = False
            tmrEnemyAttack.Enabled = False
            tmrCharAttack.Enabled = False
            lblEnemyLife.Visible = False
            
            'NEWFORM
            Call levelLoad("level2s.bmp")
            
            Call showUserText("What's This?  She seems to have dropped a coke cap.", 2500)
            Call showUserText("You pick it up and it reads:", 1500)
            Call showUserText("'Congratulations!  You Are An Instant Winner.'", 2500)
            
            MsgBox ("You've defeated Edna the Evil Scientist!")
            MsgBox ("What's This?  She seems to have dropped a coke cap.")
            MsgBox ("You pick it up and it reads:")
            MsgBox ("'Congratulations!  You Are An Instant Winner.'")
            MsgBox ("New High Score (Not Really): " & points & "!")
            MsgBox ("Game Over:  Press OK to exit.")
            
            previousLevel = -1
            Unload Me
            Exit Sub
            
            'Boss doesn't regenerate
...
Public Function charCollision() As Boolean
    ...
    charOnPlatform = False
    Dim i As Integer
    If cChar.X + cChar.Width > 800 Or cChar.X < 140 Then
        If cChar.X + cChar.Width > 800 Then
            ...
            'frmLevel3a.Show
            charCollision = True
        ElseIf cChar.X < 140 Then
            If (cChar.Y > 400 Or cChar.X < 0) Then
                charCollision = True
            End If
            'NEWFORM
            'previousLevel = 2.5
...
Public Function enemy1Animate()
    ...
    'Walking Right
    If (enemy1Walking = 1) Then
        If (cEnemy1.Cell = 7) Then
            cEnemy1.Cell = 1
        ElseIf (cEnemy1.Cell < 7 And cEnemy1.Cell > 0) Then
            cEnemy1.Cell = cEnemy1.Cell + 1
        Else
            cEnemy1.Cell = 1
        End If
    'Walking Left
    ElseIf (enemy1Walking = 2) Then
        If (cEnemy1.Cell = 14) Then
            cEnemy1.Cell = 8
        ElseIf (cEnemy1.Cell < 14 And cEnemy1.Cell > 7) Then
            cEnemy1.Cell = cEnemy1.Cell + 1
        Else
            cEnemy1.Cell = 8
        End If
    ElseIf (enemy1Walking = 3) Then
        'If (cEnemy1.Cell < 7) Then
        '    cEnemy1.Cell = cEnemy1.Cell + 1
        'ElseIf (cChar.Cell = 7) Then
        '    cEnemy1.Cell = 1
        'ElseIf (cEnemy1.Cell < 14 And cEnemy1.Cell > 7) Then
        '    cEnemy1.Cell = cEnemy1.Cell + 1
        'ElseIf (cEnemy1.Cell = 14) Then
        '    cEnemy1.Cell = 8
        'End If
    End If
    'MsgBox ("enemyAnim newCell: " & cEnemy1.Cell)
...
Public Function UseItem(ObjCollidedWith As Integer)
    ...
    'NEWFORM
    itemname = Level2xS_Items(ObjCollidedWith).ItemType
    ...
    Select Case itemname
        Case "chest"
            ...
            'NEWFORM
            If (Level2xS_Items(ObjCollidedWith).Used = True) Then
            ...
            'NEWFORM
            ElseIf (Level2xS_Items(ObjCollidedWith).Used = False) Then
                ...
                'Call showUserText("The chest was empty.", 2000)
                'NEWFORM
                Level2xS_Items(ObjCollidedWith).Used = True
            End If
        Case "healthbox"
            'healthbox used
...
Private Sub tmrEnemyAttack_Timer()
    ...
    If (cEnemy1.X < cChar.X + cChar.Width And cEnemy1.X + cEnemy1.Width > cChar.X And cEnemy1.Y < cChar.Y + cChar.Height And cEnemy1.Y + cEnemy1.Height > cChar.Y) Then
        ...
        Call showUserText("You took 8 damage from Mad Scientist.", 2000)
    End If
