...
Begin VB.Form frmLevel4a 
   ...
   Begin VB.Image Image1 
      Height          =   9000
      Left            =   0
      Picture         =   "frmLevel4a.frx":1B31
      Top             =   0
      Visible         =   0   'False
      Width           =   12000
   End
End
Attribute VB_Name = "frmLevel4a"
...
Public Sub Form_Load()
    ...
    'NEWFORM
    If (previousLevel = 3.8) Then
        Unload frmLevel3h
    ElseIf (previousLevel = 4.2) Then
        Unload frmLevel4b
    End If
    ...
    If (ItemObj.UBound > 0) Then
        ...
        Do While i <= ItemObj.UBound
            If Level4x1_IntroText = False Or Level4x1_ItemDelcaration = False Then
                Call showUserText("Level Four - Laboratory Island", 3000)
                If (charHealth < 60) Then
                    charHealth = 60
                End If
                enemy1Health = 100
                points = 0
            ...
            If Level4x1_IntroText = False Then
                ...
                'NEWFORM
                ReDim Level4x1_Items(0 To i)
                'NEWFORM
                For DeclareItemArray = 0 To UBound(Level4x1_Items)
                    'NEWFORM
                    Level4x1_Items(DeclareItemArray).FileName = ItemObj(DeclareItemArray).Caption
                    Level4x1_Items(DeclareItemArray).ItemType = Mid(ItemObj(DeclareItemArray).Caption, 1, Len(ItemObj(DeclareItemArray).Caption) - 4)
                    'Level1x1_Items(DeclareItemArray).ItemType = ItemObj(DeclareItemArray).Caption
                    Level4x1_Items(DeclareItemArray).Used = False
            ...
            'End If
            'NEWFORM
            If (Level4x1_Items(i).Used = False) Then
                ...
                cItemObj(i).Cell = 1
            'NEWFORM
            ElseIf (Level4x1_Items(i).Used = True) Then
                CreateSpriteResource cResItemObj(i), App.Path & "\" & ItemObj(i).Caption, 2, 1, &HFF00FF
                CreateSprite cResItemObj(i), cItemObj(i)
                cItemObj(i).X = ItemObj(i).Left \ Screen.TwipsPerPixelX
                cItemObj(i).Y = ItemObj(i).Top \ Screen.TwipsPerPixelY
...
Public Function UseItem(ObjCollidedWith As Integer)
    ...
    'NEWFORM
    itemname = Level4x1_Items(ObjCollidedWith).ItemType
    ...
    Select Case itemname
        Case "chest"
            ...
            'NEWFORM
            If (Level4x1_Items(ObjCollidedWith).Used = True) Then
            ...
            'NEWFORM
            ElseIf (Level4x1_Items(ObjCollidedWith).Used = False) Then
                ...
                'Call showUserText("The chest was empty.", 2000)
                'NEWFORM
                Level4x1_Items(ObjCollidedWith).Used = True
            End If
        Case "healthbox"
            'healthbox used
