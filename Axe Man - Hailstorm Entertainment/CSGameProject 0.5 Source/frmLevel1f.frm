...
Begin VB.Form frmLevel1f 
   ...
   Begin VB.Image Image1 
      Height          =   9000
      Left            =   0
      Picture         =   "frmLevel1f.frx":1B31
      Top             =   0
      Visible         =   0   'False
      Width           =   12000
   End
End
Attribute VB_Name = "frmLevel1f"
...
Public Sub Form_Load()
    ...
    CreateSpriteResource cResEnemy1, App.Path & "\Tree1B.bmp", 2, 1, &HFF00FF
    ...
    If (ItemObj.UBound > 0) Then
        ...
        Do While i <= ItemObj.UBound
            If Level1x1_IntroText = False Then
                Call showUserText("Level One - Treefest", 3000)
                charHealth = 80
                enemy1Health = 300
                points = 0
                timeleft = 7314
                'NEWFORM
                Level1x1_IntroText = True
            End If
            'NEWFORM
            If Level1x6_IntroText = False Then
                Dim DeclareItemArray As Integer
                'NEWFORM
                ReDim Level1x6_Items(0 To i)
                'NEWFORM
                For DeclareItemArray = 0 To UBound(Level1x6_Items)
                    'NEWFORM
                    Level1x6_Items(DeclareItemArray).FileName = ItemObj(DeclareItemArray).Caption
                    Level1x6_Items(DeclareItemArray).ItemType = Mid(ItemObj(DeclareItemArray).Caption, 1, Len(ItemObj(DeclareItemArray).Caption) - 4)
                    'Level1x1_Items(DeclareItemArray).ItemType = ItemObj(DeclareItemArray).Caption
                    Level1x6_Items(DeclareItemArray).Used = False
            ...
            'End If
            'NEWFORM
            If (Level1x6_Items(i).Used = False) Then
                ...
                cItemObj(i).Cell = 1
            'NEWFORM
            ElseIf (Level1x6_Items(i).Used = True) Then
                CreateSpriteResource cResItemObj(i), App.Path & "\" & ItemObj(i).Caption, 2, 1, &HFF00FF
                CreateSprite cResItemObj(i), cItemObj(i)
                cItemObj(i).X = ItemObj(i).Left \ Screen.TwipsPerPixelX
                cItemObj(i).Y = ItemObj(i).Top \ Screen.TwipsPerPixelY
...
Public Function UseItem(ObjCollidedWith As Integer)
    ...
    'NEWFORM
    itemname = Level1x6_Items(ObjCollidedWith).ItemType
    ...
    Select Case itemname
        Case "chest"
            ...
            'NEWFORM
            If (Level1x6_Items(ObjCollidedWith).Used = True) Then
            ...
            'NEWFORM
            ElseIf (Level1x6_Items(ObjCollidedWith).Used = False) Then
                ...
                'Call showUserText("The chest was empty.", 2000)
                'NEWFORM
                Level1x6_Items(ObjCollidedWith).Used = True
            End If
        Case "healthbox"
            'healthbox used
...
Private Sub tmrEnemyAttack_Timer()
    ...
    If (cEnemy1.X < cChar.X + cChar.Width And cEnemy1.X + cEnemy1.Width > cChar.X And cEnemy1.Y < cChar.Y + cChar.Height And cEnemy1.Y + cEnemy1.Height > cChar.Y) Then
        ...
        Call showUserText("You took 3 damage from Tree.", 2000)
    End If
