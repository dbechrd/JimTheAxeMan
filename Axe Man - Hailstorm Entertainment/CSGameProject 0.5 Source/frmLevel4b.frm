...
Begin VB.Form frmLevel4b 
   ...
   Begin VB.Image Image1 
      Height          =   9000
      Left            =   0
      Picture         =   "frmLevel4b.frx":161473
      Top             =   0
      Visible         =   0   'False
      Width           =   12000
   End
End
Attribute VB_Name = "frmLevel4b"
...
Public Sub Form_Load()
    ...
    'NEWFORM
    If (previousLevel = 4.1) Then
        Unload frmLevel4a
    ElseIf (previousLevel = 4.3) Then
        Unload frmLevel4c
    End If
    ...
    CreateSpriteResource cResEnemy1, App.Path & "\Tree4a1.bmp", 2, 1, &HFF00FF
    ...
    If (ItemObj.UBound > 0) Then
        ...
        Do While i <= ItemObj.UBound
            ...
            End If
            'NEWFORM
            If Level4x2_IntroText = False Then
                Dim DeclareItemArray As Integer
                'NEWFORM
                ReDim Level4x2_Items(0 To i)
                'NEWFORM
                For DeclareItemArray = 0 To UBound(Level4x2_Items)
                    'NEWFORM
                    Level4x2_Items(DeclareItemArray).FileName = ItemObj(DeclareItemArray).Caption
                    Level4x2_Items(DeclareItemArray).ItemType = Mid(ItemObj(DeclareItemArray).Caption, 1, Len(ItemObj(DeclareItemArray).Caption) - 4)
                    'Level1x1_Items(DeclareItemArray).ItemType = ItemObj(DeclareItemArray).Caption
                    Level4x2_Items(DeclareItemArray).Used = False
            ...
            'End If
            'NEWFORM
            If (Level4x2_Items(i).Used = False) Then
                ...
                cItemObj(i).Cell = 1
            'NEWFORM
            ElseIf (Level4x2_Items(i).Used = True) Then
                CreateSpriteResource cResItemObj(i), App.Path & "\" & ItemObj(i).Caption, 2, 1, &HFF00FF
                CreateSprite cResItemObj(i), cItemObj(i)
                cItemObj(i).X = ItemObj(i).Left \ Screen.TwipsPerPixelX
                cItemObj(i).Y = ItemObj(i).Top \ Screen.TwipsPerPixelY
...
Public Function UseItem(ObjCollidedWith As Integer)
    ...
    'NEWFORM
    itemname = Level4x2_Items(ObjCollidedWith).ItemType
    ...
    Select Case itemname
        Case "chest"
            ...
            'NEWFORM
            If (Level4x2_Items(ObjCollidedWith).Used = True) Then
            ...
            'NEWFORM
            ElseIf (Level4x2_Items(ObjCollidedWith).Used = False) Then
                ...
                'Call showUserText("The chest was empty.", 2000)
                'NEWFORM
                Level4x2_Items(ObjCollidedWith).Used = True
            End If
        Case "healthbox"
            'healthbox used
