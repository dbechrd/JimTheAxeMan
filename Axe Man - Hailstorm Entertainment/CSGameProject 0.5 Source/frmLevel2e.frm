...
Begin VB.Form frmLevel2e 
   ...
   Begin VB.Image Image1 
      Height          =   9000
      Left            =   0
      Picture         =   "frmLevel2e.frx":1B31
      Top             =   0
      Visible         =   0   'False
      Width           =   12000
   End
End
Attribute VB_Name = "frmLevel2e"
...
Public Sub Form_Load()
    ...
    CreateSpriteResource cResEnemy1, App.Path & "\Tree2B.bmp", 2, 1, &HFF00FF
    ...
    If (ItemObj.UBound > 0) Then
        ...
        Do While i <= ItemObj.UBound
            ...
            End If
            'NEWFORM
            If Level2x5_IntroText = False Then
                Dim DeclareItemArray As Integer
                'NEWFORM
                ReDim Level2x5_Items(0 To i)
                'NEWFORM
                For DeclareItemArray = 0 To UBound(Level2x5_Items)
                    'NEWFORM
                    Level2x5_Items(DeclareItemArray).FileName = ItemObj(DeclareItemArray).Caption
                    Level2x5_Items(DeclareItemArray).ItemType = Mid(ItemObj(DeclareItemArray).Caption, 1, Len(ItemObj(DeclareItemArray).Caption) - 4)
                    'Level1x1_Items(DeclareItemArray).ItemType = ItemObj(DeclareItemArray).Caption
                    Level2x5_Items(DeclareItemArray).Used = False
            ...
            'End If
            'NEWFORM
            If (Level2x5_Items(i).Used = False) Then
                ...
                cItemObj(i).Cell = 1
            'NEWFORM
            ElseIf (Level2x5_Items(i).Used = True) Then
                CreateSpriteResource cResItemObj(i), App.Path & "\" & ItemObj(i).Caption, 2, 1, &HFF00FF
                CreateSprite cResItemObj(i), cItemObj(i)
                cItemObj(i).X = ItemObj(i).Left \ Screen.TwipsPerPixelX
                cItemObj(i).Y = ItemObj(i).Top \ Screen.TwipsPerPixelY
...
Public Function UseItem(ObjCollidedWith As Integer)
    ...
    'NEWFORM
    itemname = Level2x5_Items(ObjCollidedWith).ItemType
    ...
    Select Case itemname
        Case "chest"
            ...
            'NEWFORM
            If (Level2x5_Items(ObjCollidedWith).Used = True) Then
            ...
            'NEWFORM
            ElseIf (Level2x5_Items(ObjCollidedWith).Used = False) Then
                ...
                'Call showUserText("The chest was empty.", 2000)
                'NEWFORM
                Level2x5_Items(ObjCollidedWith).Used = True
            End If
        Case "healthbox"
            'healthbox used
...
Private Sub tmrEnemyAttack_Timer()
    ...
    If (cEnemy1.X < cChar.X + cChar.Width And cEnemy1.X + cEnemy1.Width > cChar.X And cEnemy1.Y < cChar.Y + cChar.Height And cEnemy1.Y + cEnemy1.Height > cChar.Y) Then
        charHealth = charHealth - 4
        Call showUserText("You took 4 damage from Tree.", 2000)
    End If
