...
Begin VB.Form frmLevel2c 
   ...
   Begin VB.Image Image1 
      Height          =   9000
      Left            =   0
      Picture         =   "frmLevel2c.frx":161473
      Top             =   0
      Visible         =   0   'False
      Width           =   12000
   End
End
Attribute VB_Name = "frmLevel2c"
...
Public Sub Form_Load()
    ...
    'NEWFORM
    If (previousLevel = 2.2) Then
        Unload frmLevel2b
    ElseIf (previousLevel = 2.4) Then
        Unload frmLevel2d
    End If
    ...
    If (ItemObj.UBound > 0) Then
        ...
        Do While i <= ItemObj.UBound
            ...
            End If
            'NEWFORM
            If Level2x3_IntroText = False Then
                Dim DeclareItemArray As Integer
                'NEWFORM
                ReDim Level2x3_Items(0 To i)
                'NEWFORM
                For DeclareItemArray = 0 To UBound(Level2x3_Items)
                    'NEWFORM
                    Level2x3_Items(DeclareItemArray).FileName = ItemObj(DeclareItemArray).Caption
                    Level2x3_Items(DeclareItemArray).ItemType = Mid(ItemObj(DeclareItemArray).Caption, 1, Len(ItemObj(DeclareItemArray).Caption) - 4)
                    'Level1x1_Items(DeclareItemArray).ItemType = ItemObj(DeclareItemArray).Caption
                    Level2x3_Items(DeclareItemArray).Used = False
            ...
            'End If
            'NEWFORM
            If (Level2x3_Items(i).Used = False) Then
                ...
                cItemObj(i).Cell = 1
            'NEWFORM
            ElseIf (Level2x3_Items(i).Used = True) Then
                CreateSpriteResource cResItemObj(i), App.Path & "\" & ItemObj(i).Caption, 2, 1, &HFF00FF
                CreateSprite cResItemObj(i), cItemObj(i)
                cItemObj(i).X = ItemObj(i).Left \ Screen.TwipsPerPixelX
                cItemObj(i).Y = ItemObj(i).Top \ Screen.TwipsPerPixelY
...
Public Function UseItem(ObjCollidedWith As Integer)
    ...
    'NEWFORM
    itemname = Level2x3_Items(ObjCollidedWith).ItemType
    ...
    Select Case itemname
        Case "chest"
            ...
            'NEWFORM
            If (Level2x3_Items(ObjCollidedWith).Used = True) Then
            ...
            'NEWFORM
            ElseIf (Level2x3_Items(ObjCollidedWith).Used = False) Then
                ...
                'Call showUserText("The chest was empty.", 2000)
                'NEWFORM
                Level2x3_Items(ObjCollidedWith).Used = True
            End If
        Case "healthbox"
            'healthbox used
